...
public class DiaryMain extends JFrame {
   ...
   public DiaryMain() {
      ...
      setLocationRelativeTo(null);   // 이 코드를 넣어야 윈도우 화면 가운데로 창이 뜸
   ...
   public void printDate() {
      ...
      Calendar cal = Calendar.getInstance();
      int today = cal.get(Calendar.DAY_OF_MONTH); // 현재 일
      int currentYear = cal.get(Calendar.YEAR); // 현재 년도
      int currentMonth = cal.get(Calendar.MONTH); // 현재 월
      ...
      for (int i = 0; i < dateBoxAr.length; i++) {
         ...
         if (i >= startDay && n <= lastDate) {
            dateBoxAr[i].updateDate(Integer.toString(n)); // 이거 n을 넣어야 함

            JButton dateButton = dateBoxAr[i].getDateButton(); // JButton 가져오기
            int clickedDate = n; // 클릭한 날짜 가져오기

            // 특정 날짜 버튼에 ActionListener 추가
            dateButton.addActionListener(new ActionListener() {
               @Override
               public void actionPerformed(ActionEvent e) {
                  // 클릭한 버튼의 날짜를 콘솔에 출력
                  System.out.println(
                        "클릭한 날짜: " + yy + "-" + StringManager.getZeroString(mm + 1) + "-" + clickedDate);
               }
            });
            ...
            // 여기 더 수정해야함
            dateBoxAr[i].addActionListener(new ActionListener() {
               @Override
               public void actionPerformed(ActionEvent e) {

                  JButton clickedButton = (JButton) e.getSource();
                  int clickedDate = Integer.parseInt(clickedButton.getText());
                  ...
                  // 클릭한 날짜의 년도, 월, 일을 출력
                  // System.out.println("클릭한 날짜의 년도: " + yy);
                  // System.out.println("클릭한 날짜의 월: " + (mm + 1));
                  // System.out.println("클릭한 날짜의 일: " + clickedDate);
                  
                  // 클릭한 버튼의 날짜를 콘솔에 출력
                  String selectedDate = yy + "-" + StringManager.getZeroString(mm + 1) + "-" + clickedDate;
                  System.out.println("클릭한 날짜: " + selectedDate);
                  
                  if (listFrame == null) {
                      // If not, create a new instance
                      listFrame = new List();
                      // Use the existing instance for subsequent actions
                  }
                  
                  listFrame.displaySelectedDate(selectedDate);
                  
                  

                  if (f == null) {

                     // 어떤 날짜를 클릭했는지 알려주는 알림창 (더 수정 필요함)
                     f = new Frame("알림");
                     f.setSize(300, 100);
                     ...
                     JPanel z = new JPanel();
                     z.setPreferredSize(new Dimension(300, 10)); // 사이즈
                     z.setBackground(Color.white); // 배경색
                     f.add(z);

                     JLabel b1 = new JLabel(yy + "년 " + StringManager.getZeroString(mm + 1) + "월 " + clickedDate + "일을 선택하셨습니다."); // 이건 수정해야함
                     f.add(b1);
   ...
   public void setBackgroundImage(BufferedImage image) {
	      ...
	      backgroundPanel.add(p_north,BorderLayout.NORTH);
	      backgroundPanel.add(p_center,BorderLayout.CENTER);
	      backgroundPanel.add(p_down,BorderLayout.SOUTH);
	   }
